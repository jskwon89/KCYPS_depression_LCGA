...
**Version:** v4.2 (2026-05-08, evening) — strict add-one rule applied uniformly: B=999 yields exactly p=.001, so reaching p<.001 requires B≥1000 (not B≥999); cluster bootstrap p=1/301=.0033 is reported as p=.003 (rounded) with a footnote noting the floor, no longer "p ≤ .003". (Earlier v4.1 changes — Table 6 rebuilt with bootstrap, Table 6b for supplement, Methods/Results add-one floor — preserved.) Primary analysis uses 9-item depression scale (suicidal-ideation item E04 excluded from trajectory measure to avoid criterion contamination with the W7 distal outcome).
...
The aligned summary file is `lcmm_R/python_vs_R_class_comparison_9item.csv`; relabeled R lcmm trajectory predictions are in `class_pred_K4_9item_LABELED.csv`. Convergence of two independently-implemented optimizers strengthens confidence in the four-class solution.
...
- **R lcmm cross-validation.** Independent re-estimation in R lcmm 2.x with 30 gridsearch starts returned K=4 BIC = 25,228.08 vs. Python EM K=4 BIC = 25,228.07 and modal class counts within one individual across all four classes after explicit label alignment (R class 1↔C1, R class 3↔C2, R class 2↔C3, R class 4↔C4) [`lcmm_R/python_vs_R_class_comparison_9item.csv`, `class_pred_K4_9item_LABELED.csv`].
...
- R lcmm cross-validation against Python EM.
...
### 4.9 Conclusion
A four-class developmental typology of Korean adolescent depression, estimated with a criterion-clean 9-item depression composite, reveals that the largest emerging-adult suicidal-ideation risk pool consists of those who are essentially at baseline at age 13 but rise into clinical range across the high-school transition. Time-varying hazard analysis shows that this class's risk crosses the formerly-elevated *Decreasing* class around Wave 4–5. The seven-year prospective design, BCH correction, R lcmm cross-validation, and time-varying Cox modeling converge on a single message: depression trajectories matter, screening should be longitudinal, and the high-school transition is an under-recognized intervention window in Korean adolescent mental-health surveillance.
